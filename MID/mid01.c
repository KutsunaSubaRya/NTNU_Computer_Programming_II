...
{
    char str[10005];
    sMixedNumber Stack[10005],tmp;
    char Stack_AO[10005],c;
    int ff=0;
    printf("Q: ");
    while((c=getchar()) && c!='\0' && c!=EOF && c!='\n')
    {
        if(c!=' ')
        str[ff++]=c;
    }
    int len=strlen(str), Flag=0,AO_Flag=0,deter_A=0;
    for(int i=0;i<len;i++)
    {
        int abc[5]={0},fl=0,deter=0;
        if(deter_A==1 && str[i]=='(')
        {
            deter_A=0;
            continue;
        ...
        if(str[i]=='(')
        {
            ...
            mixed_set(&Stack[Flag],abc[0],abc[1],abc[2]);
            Flag++;
        }
        if(str[i]=='*' || str[i]=='/')
        {
            //printf("Hi */\n");
            char tmp_strchar=str[i];
            Flag--;
            tmp.a=Stack[Flag].a;
            tmp.b=Stack[Flag].b;
            tmp.c=Stack[Flag].c;
            ...
            sMixedNumber tmp_abc;
            mixed_set(&tmp_abc, abc[0], abc[1], abc[2]);
            if(tmp_strchar=='*') mixed_mul(&Stack[Flag],tmp,tmp_abc);
            else mixed_div(&Stack[Flag],tmp,tmp_abc);
            Flag++;
            deter_A=1;
            continue;
        }
        if(str[i]=='+' || str[i]=='-')
        {
            Stack_AO[AO_Flag++]=str[i];
        }
        //printf("Flag= %d, AO_Flag= %d\n",Flag,AO_Flag);
        //mixed_print(Stack[Flag-1]);
    }
    //printf("Flag= %d, AO_Flag= %d\n",Flag,AO_Flag);
    Flag--;
    //printf("St= %c", Stack_AO[0]);
    while(AO_Flag!=0)
    {
        AO_Flag--;
        //printf("AO_Flag= %d\n",AO_Flag);
        if(Stack_AO[AO_Flag]=='+')
        {
            mixed_add(&Stack[Flag-1], Stack[Flag-1], Stack[Flag]);
            Flag--;
        }
        else if(Stack_AO[AO_Flag]=='-')
        {
            mixed_sub(&Stack[Flag-1], Stack[Flag-1], Stack[Flag]);
            Flag--;
        }
    }
    printf("A: ");
